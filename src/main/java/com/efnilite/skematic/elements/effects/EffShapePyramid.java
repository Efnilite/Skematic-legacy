...
@Patterns("(make|create) [a] [new] [(1¦hollow)] pyramid at %location% (with|and) [(itemtypes[s]|block[s]|pattern)] %itemtypes% (with|and) size %number%")
public class EffShapePyramid extends SkematicEffect {
    ...
    protected void execute(Event e) {
        ...
        if (location == null || blocks == null || size == null) {
            return;
        }
    ...
    public String toString(Event e, boolean debug) {
        return "create pyramid at " + expressions[0].toString(e, debug) + " with blocks " + expressions[1].toString(e, debug) + " with radius " + expressions[2].toString(e, debug);
    }
}
